...
class JNote extends JNoteElementAbstract {
    ...
    /*modified by alan so it changes the graphics color before it paints*/
	protected void renderNoteChars(Graphics2D gfx) {
		...
		if (!(note.isRest() && note.isRestInvisible())) {
			if (note.isRest() || !isHeadInverted()) {
                ...
                char[] head = new char[] {getMusicalFont().getNoteWithoutStem()};
                gfx.drawChars(head, 0, 1, (int)notePosition.getX(), (int)notePosition.getY());
			} else {
